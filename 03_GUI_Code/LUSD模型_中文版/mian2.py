...
class mywindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.areaCal.setWindowTitle('预测城市面积')
        self.suitCal = childWindow_2(self)
        self.suitCal.setWindowTitle('校准')
        self.probCal = childWindow_3(self)
        self.probCal.setWindowTitle('模拟')
        self.valid = childWindow_4(self)
        self.valid.setWindowTitle('验证')
        self.setLabel = childWindow_5(self)
        self.setLabel.setWindowTitle('Set Label')
        self.setLanduse = childWindow_6(self)
        self.setLanduse.setWindowTitle('设置土地利用类型及权重')
        self.about = childWindow_7(self)
        ...
        self.actionHelp_User.triggered.connect(self.UserGuide)
...
class childWindow(QDialog, Ui_Dialog):   # 计算方程
    ...
    def CityCounts(self, calculation, prediction, sheet1, sheet2):
        ...
        self.child.textBrowser.append("常数项:{0}".format(para[0]))
        self.child.textBrowser.append("x项系数:{0}".format(para[1:]))
    ...
    def AreaCal(self):
        ...
        print(CityCount)
        self.child.textBrowser.append("预测城市面积为: {}".format(CityCount))
        return CityCount
...
class childWindow_2(QDialog, Ui_Dialog_2):  #计算权重
    def __init__(self, input_callback=None, parent=None):
        ...
        self.childui_6.setWindowTitle("Set Land Use")
    ...
    def AddTxt(self,txtpath, labels, i):
        lines = []
        f = open(txtpath, 'r')  # your path!
        for line in f:
            lines.append(line)
        f.close()
    ...
    def SuitabilityCal(self):  # 这些设置的参数都需要接收到窗口中用户赋予的值
        ...
        for i in range(100):
            try:
                labels.append(self.child_2.tableWidget.item(i, 0).text())
            except:
                pass
        print(labels)
        ...
        labels.append('NeighborEffect')
        labels.append('LandUse')
        labels.append('OverAccury')
        labels.append('Kappa')
        txt1 = "Weight_kappa_max.txt"
        f = open(txt1, 'w+')
        np.savetxt(txt1, Weight_Serial1, fmt='%.03f', delimiter=' ')  # 写入txt文档
        self.AddTxt(txt1, labels, 0)
        f.close()
        ...
        self.child_2.textBrowser.append("所有的结果均保存在{0}目录下的{1}文件中。kappa系数最高的结果保存在该目录下的{2}文件中。".format(savepath,"weight.txt","weight_kappa_max.txt"))
...
class childWindow_4(QDialog, Ui_Dialog_4):
    ...
    def Kappa(self):
        ...
        result = 10 * Ground_Truth + Simulated_Result #实际在十位，结果在各位
        ...
        TotalNum = float(Acc_22 + Acc_11 + Acc_21 + Acc_12)
        kappa = (float(TotalNum * (Acc_22 + Acc_11) - (float(Acc_21 + Acc_11) * (Acc_12 + Acc_11) +
                                                       float(Acc_21 + Acc_22) * (Acc_12 + Acc_22)))) / (
                            TotalNum ** 2 - (float(Acc_21 + Acc_11) * (Acc_12 + Acc_11) +
                                             float(Acc_21 + Acc_22) * (Acc_12 + Acc_22)))

        TotalNum = Acc_22 + Acc_11 + Acc_21 + Acc_12
        OverAccury = float(Acc_22 + Acc_11) / TotalNum

        self.child_4.textBrowser.append("kappa = {}".format(kappa))
        self.child_4.textBrowser.append("总体精度 = {}".format(OverAccury))
        return kappa
...
class childWindow_6(QDialog, Ui_Dialog_6): # 设置继承性和土地利用类型
    ...
    def open_Inheritance(self):
        try:
            fileName, filetype = QFileDialog.getOpenFileName(self, "Read Excel file", "D:\panxi\Desktop\yanjidata\data",
                                                             "Excel Files(*.xlsx)")
            # 设置文件扩展名过滤,注意用双分号间隔
            self.child_6.lineEdit.setText(fileName)
            Inheritance = self.child_6.lineEdit.text()
            df_I = np.array(pd.read_excel(Inheritance,'Sheet1',header=None))
            try:
                for m in range(np.shape(df_I)[0]):
                    for n in range(np.shape(df_I)[1]):
                        Inher = str(df_I[m][n])
                        item_Inher = QTableWidgetItem(Inher)
                        self.child_6.tableWidget.setItem(m, n, item_Inher)
                        print(Inher)
            except:
                pass
        except:
...
class childWindow_7(QDialog, Ui_Dialog_7):
    def __init__(self, input_callback=None, parent=None):
        ...
        path = os.getcwd()
        os.chdir(path)
        item = QGraphicsPixmapItem(QPixmap('res/about_cn.png'))
...
if __name__ == '__main__':
    ...
    splash.show()
    splash.showMessage('欢迎使用LUSD模型!',
                       Qt.AlignBottom | Qt.AlignCenter, Qt.black)

    window = mywindow()
    child = childWindow()
    child_2 = childWindow_2()
    window.actionPrediction_prediction.triggered.connect(window.areaCal.show)
    window.actionLUSD_Calibration.triggered.connect(window.suitCal.show)
    window.actionLUSD_simulation.triggered.connect(window.probCal.show)
    window.actionvalidation_Validation.triggered.connect(window.valid.show)
    window.actionHelp_About.triggered.connect(window.about.show)
    window.show()
